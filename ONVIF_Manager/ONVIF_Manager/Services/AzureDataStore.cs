...
{
    public class AzureDataStore : IDataStore<ConnectionInfo>
    {
        HttpClient client;
        IEnumerable<ConnectionInfo> items;
        ...
        public async Task<IEnumerable<ConnectionInfo>> GetItemsAsync(bool forceRefresh = false)
        {
            if (forceRefresh && IsConnected)
            {
                ...
                items = await Task.Run(() => JsonConvert.DeserializeObject<IEnumerable<ConnectionInfo>>(json));
            }
        ...
        public async Task<ConnectionInfo> GetItemAsync(string id)
        {
            if (id != null && IsConnected)
            {
                ...
                return await Task.Run(() => JsonConvert.DeserializeObject<ConnectionInfo>(json));
            }
